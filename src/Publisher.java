    ...
    private static int run(com.zeroc.Ice.Communicator communicator, String[] args)
    {
        ...
        try
        {
            ...
            while(true)
            {
            	...
            	float wSpeed = (float) (Math.random()*100);
            	short wDirection = (short) (Math.random()*10);
            	float temperature = (float) (Math.random()*100);
                monitor.report(new Measurement("Torre de Prueba", wSpeed, wDirection, temperature));
                //tick(date.format(new java.util.Date()));
